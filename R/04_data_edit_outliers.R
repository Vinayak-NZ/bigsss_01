...
# Compute IQR for SCON
Q1 <- quantile(data_imputed_pooled_all$SCON, 0.25, na.rm = TRUE)
Q3 <- quantile(data_imputed_pooled_all$SCON, 0.75, na.rm = TRUE)
IQR_value <- Q3 - Q1
...
outliers <- data_imputed_pooled_all %>% 
  filter(SCON < lower_bound | SCON > upper_bound)
